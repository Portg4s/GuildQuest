    ...
    <section className="space-y-3">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {missions.map((mission) => (
          <article
            key={mission.quiz.id}
            className="rounded-lg border border-white/10 bg-slate-900/85 p-3 shadow-xl"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2 className="mt-1 text-lg font-black text-white">{mission.quiz.title}</h2>
              </div>
              <span className="rounded-md bg-amber-300 px-2.5 py-1 text-xs font-black text-slate-950">
                Rang {mission.quiz.rank}
              </span>
            </div>
            <p className="mt-2 line-clamp-2 text-sm leading-5 text-slate-300">{mission.quiz.description}</p>
            ...
            <Button className="mt-3 h-9 w-full" onClick={() => onStartQuiz(mission)}>
